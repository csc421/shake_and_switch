...
def shake_shake_branch(x, output_filters, stride, rand_forward, rand_backward, rand_eval,
                       hparams):
...
def shake_shake_block(x, output_filters, stride, hparams):
  ...
  for branch in range(num_branches):
    r_forward, r_backward, r_eval = rand_forward_normal[branch], rand_backward_normal[branch], rand_eval[branch]
...
@registry.register_hparams
def shake_shake_piece_wise():
  hparams = shakeshake_small()
  hparams.learning_rate_decay_scheme = "piecewise"
  #hparams.train_steps = 12*10**4 #300 epochs
  CIFAR_NUM_IMAGES = 50000
  steps_per_epoch = CIFAR_NUM_IMAGES//hparams.batch_size
